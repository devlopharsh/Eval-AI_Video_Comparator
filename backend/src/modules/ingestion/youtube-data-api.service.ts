...
import { extractHashtags } from "./ingestion.utils";
...
export class YoutubeDataApiService {
  private readonly logger = new Logger(YoutubeDataApiService.name);
  private readonly apiKey: string;
  private readonly baseUrl: string;

  constructor(private readonly configService: ConfigService) {
    this.apiKey = this.configService.get<string>("youtubeDataApi.apiKey") ?? "";
    this.baseUrl = this.configService.get<string>("youtubeDataApi.baseUrl") ?? "https://www.googleapis.com/youtube/v3";
  }

  async fetchMetadata(videoUrl: string): Promise<YoutubeApiMetadata> {
    if (!this.apiKey.trim()) {
  ...
  private async fetchVideo(videoId: string) {
  ...
  private async fetchSubscriberCount(channelId: string) {
  ...
  private pickThumbnail(thumbnails?: YoutubeThumbnailSet) {
    const value =
  ...
  private parseIsoDuration(input?: string) {
    if (!input) {
      return 0;
    }
  ...
  private buildHashtags(tags: string[], description: string) {
    const normalizedTags = tags
      .map((tag) => tag.replace(/^#/, "").trim().toLowerCase())
      .filter((tag) => tag.length > 0);

    return [...new Set([...normalizedTags, ...extractHashtags(description)])];
  }
}
